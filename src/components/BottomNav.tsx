...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around h-16 bg-bg/80 backdrop-blur-md border-t border-white/5 pb-[env(safe-area-inset-bottom)]">
      {tabs.map(tab => {
        ...
        return (
          <Link
            ...
            className={`flex flex-col items-center justify-center gap-0.5 min-w-[64px] min-h-[44px] transition-colors ${
              isActive ? 'text-primary-light' : 'text-slate-400 hover:text-slate-300'
            }`}
          >
            {tab.icon}
